...
public class TwitterCityTemplate {
    ...
    {
        this.addBlocksToWorld(worldIn, pos, new TwitterCityBlockRotationProcessor(pos, placementIn), placementIn, 2);
    }
    ...
    {
        this.addBlocksToWorld(worldIn, pos, new TwitterCityBlockRotationProcessor(pos, placementIn), placementIn, flags);
    }
     ...
     * @param flags Flags to pass to {@link World#setBlockState(BlockPos, IBlockState, int)}
     */
    public void addBlocksToWorld(World worldIn, BlockPos pos, @Nullable ITwitterCityTemplateProcessor templateProcessor, PlacementSettings placementIn, int flags)
    {
        if ((!this.blocks.isEmpty() || !placementIn.getIgnoreEntities() && !this.entities.isEmpty()) && this.size.getX() >= 1 && this.size.getY() >= 1 && this.size.getZ() >= 1)
        {
            ...
            for (TwitterCityTemplate.BlockInfo template$blockinfo : this.blocks)
            {
                ...
                TwitterCityTemplate.BlockInfo template$blockinfo1 = templateProcessor != null ? templateProcessor.processBlock(worldIn, blockpos, template$blockinfo) : template$blockinfo;
            ...
            for (TwitterCityTemplate.BlockInfo template$blockinfo2 : this.blocks)
            {
