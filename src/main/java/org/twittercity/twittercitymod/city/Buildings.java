...
public class Buildings {
    ...
    public static int makeInsideCity(World world, City currentCity, List<Tweet> tweets) {
        ...
        //CityWorldData.get(world).markDirty(); probably not needed
    ...
    public static int makeBuildings(World world, City city, int tcBlocksToSpawn) {
        ...
        int x = city.getConstructionInfo().getAreaArrayFirstLoopCounter();
        int z = city.getConstructionInfo().getAreaArraySecondLoopCounter();
        int buildingID = -1;
            ...
            while (z < area[1].length) {
                if (area[x][z] >= 100 && area[x][z] <= 500) {
                    ...
                    if (buildingID >= 0 && buildingID < buildings.length) {
                        ...
                        tcBlocksToSpawn = insertBuilding(world, city, area, x, z, currentBuilding, city.getConstructionInfo().getCurrentBuildingRotation(), tcBlocksToSpawn);
                        if (tcBlocksToSpawn > 0) {
                            area[x + currentBuilding.getSizeX() - 2][z + currentBuilding.getSizeZ() - 2] = 0;
                            city.getConstructionInfo().setCurrentBuildingRotation(-1);
                            city.increaseBuildingCount();
                        } else {
                            city.getConstructionInfo().setAreaArrayFirstLoopCounter(x);
                            city.getConstructionInfo().setAreaArraySecondLoopCounter(z);
                            city.getConstructionInfo().setCurrentBuildingId(buildingID);
                            city.getConstructionInfo().setCurrentCityFinished(false);
        ...
        city.getConstructionInfo().setAreaArrayFirstLoopCounter(0);
        city.getConstructionInfo().setAreaArraySecondLoopCounter(0);
        city.getConstructionInfo().setCurrentBuildingId(buildingID);
        city.getConstructionInfo().setCurrentCityFinished(true);
    ...
    private static int insertBuilding(World world, City city, int[][] area, int x1dest, int z1dest, Building building, int rotationFixed, int tcBlocksToSpawn) {
        ...
        if (tcBlocksToSpawn > 0) {
            bp = initialBlockPos;
            rotate = -1;
            BlockHelper.spawn(city.getConstructionInfo().getBuildLast(), world);
            city.getConstructionInfo().clearBuildLast();
            CityWorldData.get(world).markDirty();
        }

        //constrData.setCurrentBuildingRotation(rotate).setCurrentConstructingBlockPos(bp);
        city.getConstructionInfo().setCurrentBuildingRotation(rotate);
        city.getConstructionInfo().setConstructingBuildingBlockPos(bp);
        CityWorldData.get(world).markDirty();
        return tcBlocksToSpawn;
    }
    ...
    private static int insertBuildingBlock(World world, City city, Building building, BlockPos currentPos, int sourceX, int sourceZ, int rotate, int tcBlocksToSpawn) {
        ...
        if ((currentPos.getY() != city.getSettings().getStartingPos().getY() + 1 || world.getBlockState(currentPos).getBlock() == Blocks.AIR)
            ...
            if (blockState == null || BlockHelper.isBlockToIgnore(blockState.getBlock())) {
                return tcBlocksToSpawn;
            }
            ...
            if (block == Blocks.BED) {
                ...
                blockState = rotate > 0 ? blockState.withProperty(BlockStandingSign.ROTATION, BlockHelper.rotateStandingSign(blockState.getValue(BlockStandingSign.ROTATION), rotate)) : blockState;
                addTextToSign(world, currentPos);
                ...
                TileEntityChest chest = (entity instanceof TileEntityChest) ? (TileEntityChest) entity : null;
                if (chest != null) {
            ...
            } else if (block == Blocks.WALL_SIGN || block == Blocks.LADDER || block == Blocks.DISPENSER || block == Blocks.FURNACE || block == Blocks.LIT_FURNACE) {
            ...
            if (!BlockHelper.needsToBeBuildedLast(block)) {
                ...
                city.getConstructionInfo().getBuildLast()
    ...
    private static void addTextToSign(World world, BlockPos blockPos) {
        // NULLPOINTEREXCEPTION
        // TileEntitySign entitySign = (TileEntitySign) world.getTileEntity(blockPos);
        // ITextComponent[] signText = entitySign.signText;

        // entitySign.signText[0] = new TextComponentString("test vasilis");
     ...
     * Add items to chests
     */
    private static void addItemsToChest(IItemHandler itemHandler) {
        ...
        for (int i = RandomHelper.nextInt(3, 6); i >= 0; i--) {
            itemHandler.insertItem(i, new ItemStack(items[RandomHelper.nextInt(items.length)], RandomHelper.nextInt(2, 8)), false);
        }
    }
